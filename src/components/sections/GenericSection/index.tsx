...
import TitleBlockLarge from '../../blocks/TitleBlockLarge';
import { Action, Badge } from '../../atoms';

export default function GenericSection(props) {
    const { elementId, colors, backgroundImage, badge, title, subtitle, text, largerText, isLargeTitle, actions = [], media, styles = {}, enableAnnotations } = props;
                ...
                {hasTextContent && (
                    ...
                    >
                        ...
                        {/* {largerText && (
                            <Markdown
                                options={{ forceBlock: true, forceWrapper: true }}
                                className={classNames('sb-markdown', 'sm:text-2xl', styles?.text ? mapStyles(styles?.text) : undefined, {
                                    'mt-6': badge?.label || title?.text || subtitle
                                })}
                                {...(enableAnnotations && { 'data-sb-field-path': '.text' })}
                            >
